...
export class TelegramService implements OnModuleInit {
    ...
    async createQrLogin(sessionId: string) {
        ...
        if (!(result instanceof Api.auth.LoginToken)) {
            this.logger.error(
                `[QR] Cannot generate LoginToken: ${result.className}`,
            );
        ...
        this.logger.log(
            `[QR] QR generated successfully: ${sessionId}`,
        );
        ...
        /**
         * Bắt đầu theo dõi UpdateLoginToken.
         *
         * Không ExportLoginToken liên tục.
         */
        this.watchLoginResult(
            sessionId,
            expiresAt,
        ).catch((error) => {
            this.logger.error(
                `[QR] Watch failed [${sessionId}]`,
     ...
     * =========================================================
     * STEP 2
     * Theo dõi UpdateLoginToken
     * =========================================================
     */
    private async watchLoginResult(
        sessionId: string,
        expiresAt: number,
    ) {
        const entry =
        ...
        if (!entry) {
            this.logger.warn(
                `[QR] Session not found: ${sessionId}`,
            );
        ...
        await new Promise<void>((resolve) => {
            ...
            const handler = async (
                ...
                this.logger.debug(
                    `[QR] Update received [${sessionId}]: ${update?.className}`,
                );
                ...
                try {
                    /**
                     * Telegram thông báo token đã được scan.
                     *
                     * Lúc này mới ExportLoginToken lần 2.
                     */
                    this.logger.log(
                        `[QR] Token scanned. Exporting login token again [${sessionId}]`,
                    );
                    ...
                    this.logger.log(
                        `[QR] Export result [${sessionId}]: ${result.className}`,
                    );
                     ...
                     */
                    this.logger.error(
                        `[QR] Unexpected login result [${sessionId}]: ${result.className}`,
                    );
    ...
    private async handleLoginSuccess(
        ...
        if (!me) {
            throw new Error(
                'Telegram login success nhưng không lấy được user',
            );
        }

        /**
         * Convert Telegram User -> object sạch
         * để lưu DB / trả API.
         */
         ...
         * Log user.
         *
         * Không log sessionString.
         */
        this.logger.log(
            `[TELEGRAM USER] ${JSON.stringify(
                telegramUser,
            )}`,
        );
         ...
         * =====================================================
         *
         * Đây là credential rất nhạy cảm.
         *
         * Không trả sessionString về frontend.
         */
        ...
        this.logger.log(
            `[QR] Telegram account stored successfully [${sessionId}]`,
        );
    }
    ...
    private async handleMigration(
        ...
        this.logger.log(
            `[QR] Telegram requires migration [${sessionId}]`,
        );

        this.logger.log(
            `[QR] Target DC: ${result.dcId}`,
        );

        /**
         * Switch Telegram client sang DC tương ứng.
         *
         * GramJS hiện expose _switchDC nội bộ,
         * nên cần cast any.
         */
        await (client as any)._switchDC(
            result.dcId,
        );

        this.logger.log(
            `[QR] Switched to DC ${result.dcId} [${sessionId}]`,
        );
        ...
        this.logger.log(
            `[QR] ImportLoginToken result [${sessionId}]: ${migrated.className}`,
        );
         ...
         */
        this.logger.debug(
            `[QR] Import result class: ${migrated.className}`,
        );
        ...
        /**
         * Trường hợp Telegram trả Authorization
         * hoặc result khác.
         */
        this.logger.error(
            `[QR] ImportLoginToken did not return LoginTokenSuccess: ${migrated.className}`,
        );
        ...
        this.sessions.set(
            sessionId,
            {
                client,
                status: 'error',
                error: `ImportLoginToken returned ${migrated.className}`,
            },
        );
    }
    ...
    private async handleLoginError(
        ...
        this.logger.error(
            `[QR] Login error [${sessionId}]: ${errorMessage}`,
            error?.stack,
        );
        ...
        if (
            ...
            this.logger.warn(
                `[QR] Telegram 2FA password required [${sessionId}]`,
            );
    ...
    getQrStatus(sessionId: string) {
        ...
        /**
         * Chỉ trả những gì frontend cần.
         *
         * Tuyệt đối không trả sessionString.
         */
        return {
            status: entry.status,
    ...
    async deleteQrSession(
        ...
        try {
            ...
            this.logger.warn(
                `[QR] Disconnect failed [${sessionId}]`,
            );
        }
